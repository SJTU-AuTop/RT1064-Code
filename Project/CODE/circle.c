...
void run_circle(){
    ...
    if(circle_type == CIRCLE_LEFT_BEGIN){
        ...
        //先丢左线后有线
        if(rpts0s_num < 25)  { none_left_line++;}
        if(rpts0s_num > 80 && none_left_line > 3)
        {
            have_left_line ++ ;
            if(have_left_line > 5)
            {
    ...
    //右环控制，前期寻左直道
        ...
        //先丢右线后有线
        if(rpts1_num < 25)  { none_right_line++;}
        if(rpts1_num > 80 && none_right_line > 3)
        {
            have_right_line ++ ;
    ...
    //走过圆环，寻左线
        ...
        if(rpts1_num > 100 && none_right_line > 3)
        { circle_type = CIRCLE_NONE;
            none_right_line = 0;}
    }
}
